...
export interface NearbyStore {
  ...
  is_live: boolean;
  distance_km: number;
  // Yayın kapalıyken "açık halinin önizlemesi" olarak arka planda gösterilir
  // (bkz. StoreLiveViewer) — kapalıyken de kullanıcı hangi mağzada olduğunu
  // görsel olarak tanısın diye.
...
export async function getNearestStores(lat: number, lng: number, limit = 20): Promise<NearbyStore[]> {
  const { data, error } = await supabase
    .from('stores')
    .select('id, name, address, lat, lng, is_live, logo_url')
    .eq('status', 'active')
    .not('lat', 'is', null)
    .not('lng', 'is', null);
