...
import { setFlashPrice, clearFlashPrice, setSpotlightProduct, subscribeToStoreOrders } from '@/lib/dampingvar';
...
export default function DealerLivePage() {
  ...
  // Kargoya vermeden önce irsaliye+fatura oluşturma adımı — bkz.
  // fixes/fix_order_documents_before_shipping.sql. app/dealer/orders
  // sayfasıyla AYNI mantık; bu sayfa ayrı bir sipariş listesi kullandığı
  // için burada da tekrarlanması gerekiyor.
  ...
  // RPC'nin write-time cevabına güvenip GRANT/RLS'e tabi read-side
  // embed'inden bağımsız kalmak için — bkz. app/dealer/orders/page.tsx
  // içindeki aynı isimli state'in yorumu.
  ...
  // Realtime — sipariş geldiğinde veya durumu değiştiğinde (ör. lojistik/
  // admin panelinden) otomatik yenile. Ortak lib/dampingvar.ts'teki
  // subscribeToStoreOrders() kullanılıyor — bu sayede aynı kanal aboneliği
  // kodu app/dealer/orders/page.tsx ve burada tekrarlanmıyor.
  useEffect(() => {
    if (!store) return;
    return subscribeToStoreOrders(store.id, () => refresh(store));
  }, [store, refresh]);
                ...
                <div key={o.id} className="rounded-lg p-2.5 text-xs" style={{ background: '#0B1220', border: '1px solid #1E2A42' }}>
                  ...
                  {shipFormFor === o.id && (
                    (invoice && deliveryNote) || docsReady.has(o.id) ? (
                      ...
                      // irsaliye+fatura oluşturma adımı (bkz. app/dealer/
                      // orders/page.tsx'teki aynı akış). Bayi numarayı
                      // kendi girebilir, boş bırakırsa otomatik üretilir.
