...
- `/supplier`, `/admin` — **taşındı** (bu not önceden "henüz taşınmadı"
  diyordu, güncel kod tabanıyla artık uyuşmuyordu, düzeltildi). Gerçek
  işlevsellik `dashboard.html` → `modules/supplier.js` /
  `modules/catalog-admin.js`'ten Next.js'e geçti:
  - `/admin` → `app/admin/page.tsx` (4 sekme: Katalog Onayı, Komisyon
    Oranları, Ürün Önerileri, Bayi Kazançları) + ayrı alt sayfalar
    (`app/admin/auctions`, `app/admin/catalog`, `app/admin/finance`,
    `app/admin/suggestions`).
  - `/supplier` → `app/supplier/page.tsx` (7 sekme: Gelen Talepler,
    Tekliflerim, Sevkiyat Durumu, Stok Yönetimi, Yeni Ürün Öner,
    Reddedilenler, Eksik Siparişler).

  **Next.js'e HÂLÂ taşınmamış olanlar** (kod tarandı — `find app -type d`
  ile teyit edildi, aşağıdakiler için `app/` altında hiçbir karşılık yok):
  - `modules/dealer-performance.js` — bayinin aylık performans geçmişi
    (salt okunur, `public.dealer_monthly_performance`'tan).
  - `modules/franchise.js` — franchise/süpervizör rolü paneli (bayi
    listesi + `seller_stats`, tamamen salt okunur). Bu rol için
    `middleware.ts`'te bile henüz bir `/franchise` rotası tanımlı değil.

  `modules/registry.js` ayrı bir durum — bir "panel" değil, legacy
  `dashboard.html`'in modül-kayıt/routing altyapısı; doğrudan bir
  Next.js karşılığı olmayacak, yukarıdaki iki modül taşındıktan sonra
  retire edilebilir hale gelecek.
