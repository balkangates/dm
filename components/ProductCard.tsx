...
export default function ProductCard({
  ...
  // "Sor" — mağazayla özel mesajlaşma panelini bu ürün bağlamıyla açar
  // (bkz. components/StoreDirectMessage.tsx). Verilmezse buton gizlenir.
  onAsk?: (p: StoreProduct) => void;
  ...
  // Video popup'ı hangi kartın üzerinde açılacağını bilsin diye, tetikleyen
  // "izle" butonunun o anki ekran konumu (bkz. openDetail). Portal ile
  // document.body'ye render edildiği için (CSS transform/overflow'a
  // hapsolmasın diye — bkz. VideoPopupModal.tsx), konumunu bu koordinatlara
  // göre kendi hesaplıyor.
  const [videoAnchor, setVideoAnchor] = useState<{ top: number; left: number; width: number } | null>(null);
  // LiveStream çerçevesinin (video + overlay kutusu) ekran sınırları —
  // popup'ın bu kutunun DIŞINA (sayfa kenar boşluğuna) taşmaması için.
  // Bkz. app/store/[storeId]/page.tsx → data-live-frame.
  const [videoFrame, setVideoFrame] = useState<{ top: number; left: number; right: number; bottom: number } | null>(null);
  ...
  const openDetail = async (e: MouseEvent<HTMLButtonElement>) => {
    // Popup'ın "kartın üzerinde" açılması için, tetikleyen butonun içinde
    // olduğu kart kutusunun (en yakın .relative sarmalayıcı — görsel +
    // buton bloğu) ekran koordinatlarını yakalıyoruz.
    const cardBox = e.currentTarget.closest('.relative') as HTMLElement | null;
    const rect = (cardBox ?? e.currentTarget).getBoundingClientRect();
    setVideoAnchor({ top: rect.top, left: rect.left, width: rect.width });

    const frameEl = e.currentTarget.closest('[data-live-frame]') as HTMLElement | null;
    if (frameEl) {
      const fr = frameEl.getBoundingClientRect();
      setVideoFrame({ top: fr.top, left: fr.left, right: fr.right, bottom: fr.bottom });
    } else {
      setVideoFrame(null);
    }
  ...
  if (compact) {
    return (
      ...
      >
          ...
          <div className="flex gap-1 mt-1 lg:mt-1.5">
            ...
            {onAsk && (
              <button
                ...
                title="Bu ürünü sor"
                className="shrink-0 rounded py-0.5 lg:py-1 px-1.5 text-[8px] lg:text-[9px] font-extrabold"
                style={{ background: 'rgba(56,189,248,0.14)', color: '#38BDF8', border: '1px solid rgba(56,189,248,0.4)' }}
              >
        ...
        {videoUrl && (
          <VideoPopupModal videoUrl={videoUrl} title={product.name} anchor={videoAnchor} frame={videoFrame} onClose={() => { setVideoUrl(null); setVideoAnchor(null); setVideoFrame(null); }} />
        )}
      </div>
    );
  }
        ...
        <div className="flex gap-1.5 mt-2.5">
          ...
          {onAsk && (
            <button
              onClick={() => onAsk(product)}
              title="Bu ürünü sor"
              className="shrink-0 rounded-lg py-2 px-3 text-xs font-extrabold flex items-center justify-center"
              style={{ background: 'rgba(56,189,248,0.14)', color: '#38BDF8', border: '1px solid rgba(56,189,248,0.4)' }}
            >
              <i className="fas fa-question" />
            </button>
