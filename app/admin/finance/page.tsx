...
import { loadFinanceSummary, loadDealerEarnings, markEarningPaid, fmtMoney, monthLabel } from '@/lib/admin';
...
export default function AdminFinancePage() {
  ...
  // NOT: dealer_earnings.payment_method kolonu DB'de bir CHECK kısıtına
  // sahip — yalnızca 'USDT' | 'bank' | 'wallet' kabul ediyor. Önceden burada
  // 'havale'/'virman'/'nakit' gönderiliyordu; bu değerler kısıta takılıp
  // markEarningPaid() DB hatasıyla başarısız oluyordu (buton hiçbir zaman
  // gerçekten çalışmıyordu). Aşağıdaki seçenekler şemayla eşleşiyor.
  const handleMarkPaid = async (id: string) => {
    const method = payMethod[id] || 'bank';
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-[#5E7090] border-b border-[#2A3650]">
                ...
                <th className="p-2.5">Referans Kom.</th>
            ...
            <tbody>
              {summary.filter((s) => s.order_count > 0).map((s) => (
                <tr key={s.store_id} className="border-b border-[#1E2A42]">
                  ...
                  <td className="p-2.5 text-[#A855F7] font-mono">
                    {Number(s.referral_commission) > 0 ? `₺${Number(s.referral_commission).toLocaleString('tr-TR')}` : '—'}
                  </td>
          ...
          <div className="space-y-2">
            {earnings.map((e) => (
              <div key={e.id} className="rounded-xl p-3 flex items-center justify-between flex-wrap gap-2" style={CARD}>
                <div>
                  <p className="text-white text-sm font-bold">{e.stores?.name} — {monthLabel(e.period_year, e.period_month)}</p>
                  <p className="text-[#5E7090] text-xs font-mono">
                    Komisyon {fmtMoney(e.gross_commission)} + Referans {fmtMoney(e.referral_bonus)} = <span className="text-white font-bold">{fmtMoney(e.total_payable)}</span>
                    {' · '}
