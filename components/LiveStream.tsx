...
import { useLiveChatConversation } from '@/lib/hooks/useLiveChatConversation';
...
export default function LiveStream({
  ...
  // Hook, o mağazanın sohbetini bulur/oluşturur VE giriş yapan kullanıcıyı
  // conversation_participants'a atomik olarak ekler (bkz.
  // lib/hooks/useLiveChatConversation.ts + fixes/fix_live_chat_participants_v3.sql).
  // storeId her zaman zorunlu — LiveStream artık storeId'siz hiçbir yerde
  // render edilmiyor, bu yüzden sabit/legacy bir fallback conversation ID'si
  // yok; conversationId null ise UI zaten "sohbet hazırlanıyor" durumunu
  // gösteriyor (aşağıdaki null kontrolleri).
  const {
    conversationId,
    error: conversationError,
    retry: retryConversation,
  } = useLiveChatConversation(storeId);
  ...
  // Mağaza değişince eski mesajları temizle (yeni conversationId hook'tan gelecek).
  useEffect(() => {
    setMessages([]);
  }, [storeId]);
  ...
  useEffect(() => {
    ...
    supabase.from('messages').insert({
      ...
      if (error) {
        console.error('[LiveStream] katılım mesajı gönderilemedi:', error.message, error.code);
        // Stale conversationId (23503) ile bir daha denenmesin diye tekrar
        // çözümleme tetikle ve tekrar gönderilebilsin diye flag'i temizle.
        if (error.code === '23503') {
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (error) {
      ...
      // 23503 = FK ihlali → conversationId artık DB'de yok (stale/geçersiz).
      // Hatayı GİZLEMİYORUZ (kullanıcıya gösteriyoruz) ama aynı zamanda
      // sohbeti hook üzerinden yeniden çözümlemeyi TETİKLİYORUZ ki bir
      // sonraki deneme tekrar aynı geçersiz id ile başarısız olmasın.
      if (error.code === '23503' && storeId) {
        setSendError('Sohbet bağlantısı yenileniyor, lütfen tekrar gönderin.');
        retryConversation();
      } else {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width: 'min(68%, 380px)', bottom: 'calc(56px + env(safe-area-inset-bottom, 0px))' }}
      >
      ...
      {/* ── Chat giriş kutusu — footer ──────────────────────────────────────────
          Mobilde bu konteyner ekranın en altına sabitlendiğinde, telefonun
          alt "home indicator" / gesture çubuğunun ARKASINDA kalmaması için
          env(safe-area-inset-bottom) ile ekstra alt boşluk ekleniyor
          (iOS Safari / Android Chrome'da fark edilir bir sorundur). ── */}
      <div
        className="absolute bottom-0 left-0 right-0 z-30 px-3 pt-2.5"
        style={{
      ...
      >
        ...
        {/* Sohbet odası oluşturulamadıysa (RPC/ağ hatası) — kullanıcı giriş
            yapmış olsa bile conversationId gelmemiş olabilir. */}
        {user && !conversationId && conversationError && conversationError !== 'AUTH_REQUIRED' && (
          <div className="mb-1.5 px-3 py-1.5 rounded-lg text-[11px] font-mono text-amber-300 flex items-center justify-between gap-2"
            style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)' }}>
            <span><i className="fas fa-triangle-exclamation mr-1.5" />Sohbet odasına bağlanılamadı.</span>
            <button onClick={retryConversation} className="underline shrink-0">Tekrar dene</button>
          </div>
        )}
