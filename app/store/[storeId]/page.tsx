...
import StoreDirectMessage, { type DirectMessageOpenRequest } from '@/components/StoreDirectMessage';
...
export default function StorePage() {
  ...
  const [dmOpenRequest, setDmOpenRequest] = useState<DirectMessageOpenRequest | null>(null);
  ...
  // "Sor" akışının doğru kişiye (mağaza sahibi profiles.id) gitmesi için
  // stores.owner_id gerekli — StoreDirectMessage bunu receiver_id olarak kullanır.
  useEffect(() => {
    supabase.from('stores').select('owner_id').eq('id', storeId).single()
      .then(({ data }) => setStoreOwnerId(data?.owner_id ?? null));
  }, [storeId]);
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8 space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[65%_35%] gap-5">
        <div className="relative flex flex-col" data-live-frame>
          <StoreLiveViewer storeId={store.store_id} storeName={store.name} initialIsLive={store.is_live} logoUrl={store.logo_url} />
          ...
          <StoreShopOverlay
            ...
            onAsk={(p) => setDmOpenRequest({ nonce: Date.now(), productId: p.id, productName: p.name })}
          />
              ...
              return (
                <div key={o.id} className="rounded-lg p-3" style={{ background: '#0B1220', border: '1px solid #1E2A42' }}>
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <button
                      onClick={() => setDmOpenRequest({
                        nonce: Date.now(),
                        orderId: o.id,
                        orderLabel: `#${String(o.id).slice(0, 8)}`,
                      })}
                      title="Bu siparişi sor"
      ...
      {/* Müşteri ↔ mağaza ÖZEL mesajlaşma paneli — "Sor" linkleri
          (ürün kartı / Siparişlerim) bunu ilgili bağlamla açar. Canlı
          yayın sohbetinden (yukarıdaki LiveStream) tamamen ayrı bir
          konuşma kaydı üzerinden çalışır. */}
      {user && (
        <StoreDirectMessage
          storeId={store.store_id}
          storeName={store.name}
          storeOwnerId={storeOwnerId}
          openRequest={dmOpenRequest}
        />
      )}
    </main>
  );
