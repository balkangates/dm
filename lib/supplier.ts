...
export interface SupplierCatalogProduct {
  ...
  visibility_scope: 'self' | 'all' | 'selected';
...
export async function fetchMyCatalogProducts(supplierId: string): Promise<SupplierCatalogProduct[]> {
  const { data } = await supabase
    ...
    .select('*, product_variants(*), brands(id,name,logo_url)')
    .eq('supplier_id', supplierId)
    .order('created_at', { ascending: false });
  return data || [];
}
...
// ── Faz 5: marka (brand) ────────────────────────────────────────────────
// "Markalarım" — bu tedarikçinin ürünlerinde şu an kullanılan markalar
// (brands tablosunda supplier_id yok — global/paylaşımlı tablo — bu
// yüzden "bana ait" olan, kendi ürünlerimde kullandıklarım anlamına gelir).
export async function fetchMyBrands(supplierId: string): Promise<Brand[]> {
  ...
  if (error) {
    console.error('[supplier] markalarım yüklenemedi:', error);
    return [];
  }
  const map = new Map<string, Brand>();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (data || []).forEach((row: any) => {
    if (row.brands) map.set(row.brands.id, row.brands);
  });
  return Array.from(map.values());
}

// Seçim listesi için TÜM markalar (yeni ürün önerirken var olan bir
// markayı seçebilmek için — sadece "kendi" markalarıyla sınırlı değil).
export async function fetchAllBrands(): Promise<Brand[]> {
  const { data, error } = await supabase.from('brands').select('id,name,logo_url').eq('is_active', true).order('name');
  if (error) {
    console.error('[supplier] marka listesi yüklenemedi:', error);
    return [];
  }
  return data || [];
}

// Yeni marka ekle. brands.name UNIQUE olduğu için aynı isim tekrar
// eklenmeye çalışılırsa DB constraint hatası fırlatır — çağıran taraf
// bunu "bu marka zaten var" mesajına çevirmeli.
export async function createBrand(name: string, logoUrl?: string): Promise<Brand> {
  const trimmed = name.trim();
  if (!trimmed) throw new Error('Marka adı gerekli.');
  const { data, error } = await supabase
    .from('brands')
    .insert({ name: trimmed, logo_url: logoUrl?.trim() || null })
...
export async function proposeNewProduct(
  supplierId: string,
  payload: { name: string; category_id: string; subcategory_id: string | null; suggested_price: number; brand_id: string | null },
) {
...
// ── Faz 4: ürün görünürlüğü (kendisi/herkes/seçili bayiler) ─────────────
...
// Bu ürünü (catalog_products) şu an satan tüm bayilerin store_products
// satırları — sadece visibility_scope='all'/'selected' + tedarikçi-bayi
// (dual-role) hesaplar için RLS bunu güncellemeye izin veriyor
// (bkz. fix_phase21_supplier_scoped_price_update.sql). Salt-okunur
// listeleme her zaman kendi ürünün olduğu için zaten mümkün.
export async function fetchSharedListingsForProduct(catalogProductId: string): Promise<SharedListing[]> {
  const { data, error } = await supabase
    .from('store_products')
    .select('id, store_id, price, stock_qty, is_active, stores(name)')
...
// NOT: .select() olmadan .update() yapılınca RLS reddi sessiz kalır
// (0 satır günceller, hata dönmez) — bkz. fix_phase20/21 notları.
// .select().single() ile bunu görünür bir hataya çeviriyoruz.
export async function updateSharedListingPrice(storeProductId: string, price: number) {
  const { data, error } = await supabase
    .from('store_products')
    .update({ price })
  ...
  if (!data) {
    throw new Error(
      'Fiyat güncellenmedi (0 satır etkilendi). Bu yetki sadece tedarikçi-bayi (dual-role) ' +
      'hesaplara ve visibility_scope="all"/"selected" ürünlere tanınıyor — hesap veya ürün ayarını kontrol edin.',
    );
  }
}

export async function fetchDealerOptions(): Promise<DealerOption[]> {
  const { data, error } = await supabase.rpc('get_dealer_options');
  if (error) throw error;
  return data ?? [];
}

export async function fetchGrantedDealerIds(catalogProductId: string): Promise<string[]> {
  const { data, error } = await supabase
    .from('catalog_product_dealer_access')
    .select('dealer_id')
    .eq('catalog_product_id', catalogProductId);
  if (error) throw error;
...
/** scope='selected' ise dealerIds ile mevcut listeyi tam olarak eşitler
 * (var olanları silmeden sadece eklemek yerine, seçimden çıkarılanları da
 * kaldırır — kullanıcı ne görüyorsa o kaydedilsin diye). */
export async function updateProductVisibility(
  catalogProductId: string,
  scope: 'self' | 'all' | 'selected',
  dealerIds: string[],
) {
  const { error: e1 } = await supabase.from('catalog_products').update({ visibility_scope: scope }).eq('id', catalogProductId);
  if (e1) throw e1;

  if (scope !== 'selected') return;

  const current = await fetchGrantedDealerIds(catalogProductId);
  const toAdd = dealerIds.filter((id) => !current.includes(id));
  const toRemove = current.filter((id) => !dealerIds.includes(id));

  if (toAdd.length > 0) {
    const { error } = await supabase
      .from('catalog_product_dealer_access')
      .insert(toAdd.map((dealer_id) => ({ catalog_product_id: catalogProductId, dealer_id })));
    if (error) throw error;
  }
